        ...
        for v in self.vehicles:
            if v.orientation == 'H':
                if v.xPos - 1 >= 0 and gameBoard[v.yPos][v.xPos - 1] == ' ':
                    ...
                    newVehicles = self.vehicles[:]
                    newVehicles.remove(v)
                    newVehicles.append(newVehicle)
                    yield GameBoard(newVehicles)
...
def loadFile(inFile):
        ...
        for line in inFile:
            ...
            vehicles.append(Vehicle(name, int(x), int(y), orientation))
        print("Ended load")
        return GameBoard(vehicles)
